...
struct fs_file http_read_parser(char *buf1, uint16_t in_data_len, char *out_buf, uint16_t *left_data, bool *ok) {
    struct fs_file file = {NULL, 0, 0, NULL};
    *left_data = 0;
    *ok = true; // запрос полностью обработан, можно посылать ответ
    char *const start_buf = buf1;

    trip_space_char(&buf1);
    method_t method = get_method(&buf1);
    if (method == M_NOT_SUP) {
        error_501(out_buf);
        return file;
    }

    struct buffer uri = get_uri(&buf1);
    ...
    ext_t ext = E_NOT_SUP;
    ext = get_data_extension_request(uri.data, uri.len);
    if (ext == E_NOT_SUP) {
        error_415(out_buf);
        return file;
    }

    version_t http_version = get_http_version(&buf1);
    ...
    file = method_handler(ext, uri.data, uri.len, out_buf, body, chunk_len, method);
    return file;
}
