...
void test_search_method() {
    ...
    struct buffer buf = searchInBuffer(&uri, where_method);
...
void test_search_uri() {
    ...
    struct buffer buf = searchInBuffer(&uri, where_uri);
...
void test_search_version() {
    char *uri = "HTTP1.1\r\n";
    struct buffer buf = searchInBuffer(&uri, where_version);
